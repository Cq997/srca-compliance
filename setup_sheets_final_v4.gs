...
function getComplaintsStats(ss) {
  ...
  // r[10] = النتيجة (وفق ترتيب appendComplaint)
  const underAction = rows.filter(r => r[10] === "تحت الإجراء").length;
  const suspicion   = rows.filter(r => r[10] === "اشتباه وجود مخالفة").length;
  const noViolation = rows.filter(r => r[10] === "عدم وجود مخالفة").length;
  const notProven   = rows.filter(r => r[10] === "عدم ثبوت المخالفة").length;
...
function getComplaints(ss) {
  ...
  // ترتيب الأعمدة يطابق appendComplaint:
  // 0:id, 1:type, 2:source, 3:refNumber, 4:subject,
  // 5:sectorName, 6:centerName, 7:inspectorName, 8:violatorName,
  // 9:detectionDate, 10:result, 11:madaadNumber, 12:resultDate,
  // 13:description, 14:notes, 15:createdAt
  const rows = sheet.getDataRange().getValues().slice(2);
  const complaints = rows.filter(r => r[0]).map(r => ({
    id:            String(r[0] || ""),
    type:          String(r[1] || ""),
    source:        String(r[2] || ""),
    refNumber:     String(r[3] || ""),
    subject:       String(r[4] || ""),
    sectorName:    String(r[5] || ""),
    centerName:    String(r[6] || ""),
    inspectorName: String(r[7] || ""),
    violatorName:  String(r[8] || ""),
    detectionDate: r[9] ? (r[9] instanceof Date ? r[9].toISOString().split('T')[0] : String(r[9])) : "",
    result:        String(r[10] || ""),
    madaadNumber:  String(r[11] || ""),
    resultDate:    r[12] ? (r[12] instanceof Date ? r[12].toISOString().split('T')[0] : String(r[12])) : "",
    description:   String(r[13] || ""),
    notes:         String(r[14] || ""),
    createdAt:     r[15] ? (r[15] instanceof Date ? r[15].toISOString() : String(r[15])) : ""
  }));
  return { complaints };
}
...
// ═══════════════════════════════════════════════════════════════
// دالة ترحيل بيانات ورقة الشكاوى إلى الهيكل الجديد
// شغّل هذه الدالة مرة واحدة فقط إذا كانت الورقة بترتيب قديم
// ═══════════════════════════════════════════════════════════════
function migrateComplaintsSheet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const oldSheet = ss.getSheetByName("الشكاوى_والمخالفات");
  if (!oldSheet) {
    SpreadsheetApp.getUi().alert("لم يُوجد ورقة الشكاوى_والمخالفات");
    return;
  }

  // قراءة جميع البيانات الموجودة
  const allData = oldSheet.getDataRange().getValues();
  if (allData.length <= 2) {
    SpreadsheetApp.getUi().alert("الورقة فارغة أو تحتوي على رؤوس فقط. لا يوجد ما يُرحَّل.");
    return;
  }

  // قراءة رؤوس الأعمدة الحالية (الصف الثاني)
  const currentHeaders = allData[1].map(h => String(h).trim());
  
  // الرؤوس المستهدفة (تتطابق مع appendComplaint)
  const targetHeaders = [
    "الرقم", "النوع", "المصدر", "رقم المعاملة",
    "الموضوع", "القطاع", "المركز",
    "القائم بالفحص", "اسم المعني/المخالف",
  ...
  // خريطة من اسم العمود إلى فهرسه في الورقة الحالية
  const headerMap = {};
  currentHeaders.forEach((h, i) => { if (h) headerMap[h] = i; });

  // قراءة صفوف البيانات (من الصف الثالث فصاعداً)
  const dataRows = allData.slice(2).filter(r => r[0] || r[1]);

  // إنشاء ورقة جديدة مؤقتة
  const tempName = "الشكاوى_والمخالفات_جديدة";
  ...
  tempSheet.setTabColor("#C0392B");
  setSheetTitle(tempSheet, "سجل الشكاوى والمخالفات — منظومة إدارة الالتزام");
  setHeaderRow(tempSheet, 2, targetHeaders, "#C0392B");

  // تحويل كل صف إلى الترتيب الجديد
  const newRows = dataRows.map(row => {
    return targetHeaders.map(header => {
      const oldIdx = headerMap[header];
      if (oldIdx !== undefined) return row[oldIdx] || "";
      // حقول لم تكن موجودة في الورقة القديمة
      return "";
    });
  });

  if (newRows.length > 0) {
    tempSheet.getRange(3, 1, newRows.length, targetHeaders.length).setValues(newRows);
  }

  // قوائم منسدلة
  tempSheet.getRange("B3:B10000").setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(["شكوى", "مخالفة"], true).build()
  );
  tempSheet.getRange("K3:K10000").setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(["اشتباه وجود مخالفة", "عدم وجود مخالفة", "عدم ثبوت المخالفة", "تحت الإجراء"], true).build()
  );

  // حذف الورقة القديمة وإعادة تسمية الجديدة
  ss.deleteSheet(oldSheet);
  tempSheet.setName("الشكاوى_والمخالفات");

  SpreadsheetApp.getUi().alert(
    "✅ تم ترحيل " + newRows.length + " سجل بنجاح إلى الهيكل الجديد!\n\n" +
    "الأعمدة الآن:\n" +
    "A: الرقم | B: النوع | C: المصدر | D: رقم المعاملة\n" +
    "E: الموضوع | F: القطاع | G: المركز\n" +
    "H: القائم بالفحص | I: اسم المعني/المخالف\n" +
    "J: تاريخ الرصد | K: النتيجة\n" +
    "L: رقم مداد | M: تاريخ النتيجة النهائية\n" +
    "N: الوصف | O: ملاحظات | P: تاريخ الإدخال"
  );
}
